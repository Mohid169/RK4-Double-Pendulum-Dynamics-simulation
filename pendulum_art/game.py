...
class PendulumArtGame:
    ...
    def update_physics(self, dt):
        if not self.paused:
            ...
            tip_screen = to_screen((x2, y2), (self.width, self.height), self.scale, offset=origin)
            ...
            if self.painting and 0 <= tip_screen[0] < self.width and 0 <= tip_screen[1] < self.height:
                pygame.draw.circle(self.canvas, self.current_color, tip_screen, self.brush_size)
