...
class DoublePendulum:
    ...
    def simulate(self, initial_state, t_max=10.0, dt=0.01):
        ...
        for i in range(n_steps-1):
            ...
            v2_squared = self.l2**2 * omega2**2 + self.l1**2 * omega1**2 + 2*self.l1*self.l2*omega1*omega2*np.cos(theta1-theta2)
            kinetic = 0.5 * self.m1 * v1_squared + 0.5 * self.m2 * v2_squared
            
            # Potential energy - zero at the pivot
            potential = -self.m1 * self.g * self.l1 * np.cos(theta1) - self.m2 * self.g * (self.l1 * np.cos(theta1) + self.l2 * np.cos(theta2))
    ...
    def create_animation(self, results, show_trace=True, save_file=None):
        ...
        sample_rate = max(1, n_frames // 300)  # Limit to ~300 frames for smoother playback
        
        anim = FuncAnimation(fig, animate, frames=range(0, n_frames, sample_rate),
